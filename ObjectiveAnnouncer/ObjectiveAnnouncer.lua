...
local DAILY, GROUP, WEEKLY, QUEST_COMPLETE, ABANDON_QUEST = _G.DAILY, _G.GROUP, _G.CALENDAR_REPEAT_WEEKLY, _G.QUEST_COMPLETE, _G.ABANDON_QUEST
...
local defaults = {
	profile = {
		...
		annSoundName = "PVPFlagCapturedHorde", annSoundFile = "Interface\\Addons\\ObjectiveAnnouncer\\Sounds\\PVPFlagCapturedHordeMono.wav",
		compSoundName = "PVPFlagCaptured", compSoundFile = "Interface\\Addons\\ObjectiveAnnouncer\\Sounds\\PVPFlagCapturedMono.wav",
		commSoundName = "GM ChatWarning", commSoundFile = "Interface\\Addons\\ObjectiveAnnouncer\\Sounds\\GM_ChatWarning.wav",
		acceptSoundName = "Hearthstone-QuestAccepted", acceptSoundFile = "Interface\\Addons\\ObjectiveAnnouncer\\Sounds\\Hearthstone-QuestingAdventurer_QuestAccepted.wav",		
		failSoundName = "Hearthstone-QuestFailed", failSoundFile = "Interface\\Addons\\ObjectiveAnnouncer\\Sounds\\Hearthstone-QuestingAdventurer_QuestFailed.wav",
			-- Out of Range Alerts --
		enableOOR = false,
	},
	char = {taskStorage = {},},
...
local function oaBuildInitialTable(event, ...)
	...
	questLogStatus["numEntries"] = tonumber(numEntries)
	questLogStatus["numQuests"] = tonumber(numQuests)

	for entryIndex = 1, numEntries do
		local title, level, questTag, suggestedGroup, isHeader, isCollapsed, isComplete, isTask, questID = GetQuestLogTitle(entryIndex)
		local frequency = 1
		if isTask then
			frequency = 2
		end
		if not isHeader then
			questLogStatus[questID] = {complete = isComplete, qLink = GetQuestLink(entryIndex), isTask = isTask}
...
local function oaMessageCreator(entryIndex, questID, objDesc, objComplete, level, suggestedGroup, isComplete, frequency)

	local divider = false
	local questLink = GetQuestLink(entryIndex)
	...
	if self.db.profile.enableOOR and (myLogIndex > 0 or isTask == "true") then	-- Is quest in our log? Also handles tasks, even if not in the log right now.
		...
		if (myObjComplete == false) and ((isTask == "false") or validTaskInfo) then	-- Don't execute if we're already done with the objective. If no saved data about a task, do something else.				
			...
			if (currentDelta > oorGroupStorage[groupMember][oorQuestID][oorBoardIndex].savedDelta) then	-- If current delta increased over previous delta, we missed an objective. If delta decreased, do nothing.
				local qlink = GetQuestLink(oorBoardIndex) or self.db.char.taskStorage[oorQuestID].taskQuestLink
			...
			for entryIndex = 1,  numEntries do
				...
				if (not isHeader) and (not questLogStatus[questID]) then
					...
					for boardIndex = 1, GetNumQuestLeaderBoards(entryIndex) do
						...
						if isTask then
							if (not self.db.char.taskStorage[questID]) then
								self.db.char.taskStorage[questID] = {taskQuestLink = GetQuestLink(entryIndex)}
							end
					...
					if isTask and self.db.profile.questTask then
						local taskMessage = L["areaentered"].." -- "..GetQuestLink(entryIndex)
		...
		--[[ Announcements Logic ]]--
			...
			--[[ Completed Quests Only ]]--
				if isComplete == 1 and isComplete ~= questLogStatus[questID].complete then
					questLogStatus[questID].complete = isComplete
					if self.db.profile.annType ==  1 then
						oaMessageCreator(entryIndex, questID, nil, true, level, suggestedGroup, isComplete, frequency)
					end
				end
				...
				for boardIndex = 1, GetNumQuestLeaderBoards(entryIndex) do
					...
					if (objComplete) and objComplete ~= questLogStatus[questID][boardIndex].complete then
						...
						if self.db.profile.annType == 2 or self.db.profile.annType == 3 then					
							oaMessageCreator(entryIndex, questID, objDesc, objComplete, level, suggestedGroup, isComplete, frequency)
						end
					end
						...
						if percent ~= questLogStatus[questID][boardIndex].description then
							if percent > 0 then
								...
								if (progressAnnCheck(percent, questID) and self.db.profile.annType == 3) or (self.db.profile.annType == 4) or (self.db.profile.annType == 5) then	-- Run pAC() first to ensure that pbThresholds[qID] stays up to date.
									...
									oaMessageCreator(entryIndex, questID, percObjDesc, objComplete, level, suggestedGroup, isComplete, frequency)
								end
...
local function oaQuestAccepted(event, ...)
	local questLogIndex = ...
	oaUpdateQuestLog()
	if self.db.profile.questAccept and (not select(8,GetQuestLogTitle(questLogIndex))) then
...
local function oaQuestTurnin(event, ...)
	...
	local numEntries, numQuests = GetNumQuestLogEntries()
	for entryIndex = 1, numEntries do
		local title,_,_,_,isHeader = GetQuestLogTitle(entryIndex)
		if not isHeader and questTitle == title then
			turnLink = GetQuestLink(entryIndex)
		end
	end
...
local function oaAutoComplete(event, ...)
	if self.db.profile.infoAutoComp then
		...
		oaMessageHandler(message, true)
		ShowQuestComplete(qID)	-- Automatically brings up the quest turn-in dialog window.
	end
end
